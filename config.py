...
@dataclass(frozen=True)
class SplitConfig:
    # Proposed-method split mode.
    # "farthest" keeps the original geometry-only split.
    # "enterprise_first" starts bisection from enterprise anchors when possible.
    mode: Literal["farthest", "enterprise_first"] = "enterprise_first"
...
class EnterpriseConfig:
    ...
    # Proposed-method center calculation.
    # "mean" keeps the original geometric/arithmetic center.
    # "qos_weighted" pulls the beam center toward enterprise users.
    center_mode: Literal["mean", "qos_weighted"] = "qos_weighted"
...
class LoadBalanceRefineConfig:
    ...
    # Proposed-method Option D behavior.
    # If True, load-balance refinement only moves eco/standard users.
    # Baselines do not call this refinement, so this does not affect baselines.
    lock_enterprise_moves: bool = True
