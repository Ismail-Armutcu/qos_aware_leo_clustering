...
"""
Thesis-final sensitivity runner.

Design goals:
- use representative user scales aligned with the main study
- use enough seeds for meaningful median / p10 / p90 summaries
- center local sensitivity sweeps around the actual baseline config
- separate local traffic sensitivity from stress testing
- use a more meaningful interaction study: theta_3db_deg x W_slots
- generate tables/plots only from the CSVs created by this script
"""
...
# =============================================================================
# SETTINGS — thesis-final defaults
# =============================================================================
SETTINGS: dict[str, Any] = {
    ...
    # Output directory
    "out_dir": "plots/sensitivity_results",
    ...
    # Representative scales for thesis plots/tables
    "n_users_list": [1000, 2000, 3000],
    "seeds": [1],

    # One-factor sweeps (centered around current baseline)
    "theta_3db_deg_values": [0.9, 1.0, 1.1],
    "elev_mask_deg_values": [20.0, 25.0, 30.0],
    "Ks_max_values": [64, 96, 128],
    "W_slots_values": [4, 6, 8],
    ...
    "demand_mbps_median_values": [6.0, 8.0, 10.0],
    "demand_mbps_median_stress_values": [12.0, 20.0, 35.0],

    # One interaction study: geometry x payload-time budget
    "interaction_theta_3db_deg_values": [0.9, 1.0, 1.1],
    "interaction_W_slots_values": [4, 6, 8],
}
...
def _build_interaction_tasks() -> list[dict[str, Any]]:
    ...
    for theta in SETTINGS["interaction_theta_3db_deg_values"]:
        for w_slots in SETTINGS["interaction_W_slots_values"]:
            updates = {
                "beam.theta_3db_deg": theta,
                "payload.W_slots": w_slots,
            }
            for n_users in SETTINGS["n_users_list"]:
                for seed in SETTINGS["seeds"]:
                    tasks.append({
                        "tag": "interaction:theta_x_Wslots",
                        "x_name": "theta_x_Wslots",
                        "x_value": f"theta={theta},W={w_slots}",
                        "theta_3db_deg": theta,
                        "W_slots": w_slots,
                        "n_users": n_users,
                        "seed": seed,
...
def _write_tables_from_csvs(out_dir: Path) -> None:
    ...
    inter_cols = [
        "tag", "n_users", "theta_3db_deg", "W_slots",
        "payload_feasible_mean",
        "K_median", "U_mean_median", "U_max_median",
        "ent_edge_pct_median", "best_m_median",
        "runtime_total_s_median",
...
def _plot_interaction_from_csvs(out_dir: Path) -> None:
    ...
    for n_users, g in interaction_summary.groupby("n_users"):
        ...
        g["W_slots"] = pd.to_numeric(g["W_slots"], errors="coerce")
        g = g.sort_values(["theta_3db_deg", "W_slots"])

        for metric, title_metric in metric_specs:
            if metric not in g.columns:
                continue
            pivot = g.pivot(index="theta_3db_deg", columns="W_slots", values=metric)
            if pivot.empty:
                continue
...
def main() -> None:
    ...
    required_cols = ["tag", "n_users", "theta_3db_deg", "W_slots"]
